{ RxDBGrid sort engine module for Sqlite3DataSet
...
uses
  Classes, SysUtils, DB, RxDBGrid, Sqlite3DS;

type

  { TSqlite3DataSetSortEngine }

  TSqlite3DataSetSortEngine = class(TRxDBGridSortEngine)
  public
...
procedure TSqlite3DataSetSortEngine.Sort(FieldName: string; ADataSet: TDataSet;
  Asc: boolean; SortOptions: TRxSortEngineOptions);
begin
  SortList(FieldName,ADataSet,Asc,SortOptions);
end;

procedure TSqlite3DataSetSortEngine.SortList(ListField: string; ADataSet: TDataSet;
...
begin
    ...
    cmd:=(ADataSet as TSqlite3DataSet).SQL;
    strlen:=length(cmd);
    ...
    with TRegExpr.Create do begin
      ...
      (ADataSet as TSqlite3DataSet).SQL:=s;
...
initialization
  RegisterRxDBGridSortEngine(TSqlite3DataSetSortEngine, 'TSqlite3DataSet');
end.
